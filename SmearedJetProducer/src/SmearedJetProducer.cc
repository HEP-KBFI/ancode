...
SmearedJetProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   //  Handle<ValueMap<float> > puJetIdRMS;
   //iEvent.getByLabel("ptRMS","puJetId", puJetIdRMS);

   std::auto_ptr<JetCollection> outJets(new JetCollection());
   
   for (unsigned int i = 0; i<jets->size(); i++) {
     std::auto_ptr<Jet> jet(new Jet( jets->at(i) ) );
     ...
     //float rms = (*puJetIdRMS)[jets->refAt(i)];
